<?php
/**
 * Template Simples de Formulário de Configurações
 * Variáveis: $settings, $grupo, $active_tab, $page_title
 */
                            ...
                            <div class="border rounded-lg p-4">
                                ...
                                <?php elseif ($setting->tipo === 'int' || $setting->tipo === 'float'): ?>
                                    <input type="number" name="<?php echo $setting->chave; ?>" value="<?php echo $setting->valor; ?>" 
                                           ...
                                           min="0" <?php echo $setting->tipo === 'float' ? 'step="0.01"' : ''; ?> required>
                                <?php elseif ($setting->tipo === 'json'): ?>
                                    <textarea name="<?php echo $setting->chave; ?>" rows="4"
                                              class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm font-mono"><?php echo $setting->valor; ?></textarea>
                                <?php else: ?>
                                ...
                                <p class="mt-2 text-xs text-gray-500">Atual: <code class="bg-gray-100 px-1 rounded"><?php echo htmlspecialchars(substr($setting->valor, 0, 50)); ?></code></p>
                            </div>
                        <?php endforeach; ?>
                    </div>
